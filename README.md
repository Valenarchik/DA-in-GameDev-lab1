...
```
Следующая большая проблема - это разобраться с Google.Apis.Sheets.v4. Скачивание простого NuGet Package не помогает. Unity просто не видит его. Чтобы исправить проблему пришлось достать dll из пакета для netstandart 2.0 и переместить в сoзданую мной папку Plugins в Assets. И все заработало. Далее я написал методы для отправки данных в мою гугл таблицу. Вот итоговый скрипт.
```cs
